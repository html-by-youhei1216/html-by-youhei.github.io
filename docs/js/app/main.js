...
const App = {
  ...
  async loadData() {
    ...
    const [lessons, exercises, snippets] = await Promise.all([
      fetch(new URL('lessons.json', dataBase)).then(r => r.json()),
      fetch(new URL('exercises.json', dataBase)).then(r => r.json()),
      fetch(new URL('snippets.json', dataBase)).then(r => r.json()),
    ]);
